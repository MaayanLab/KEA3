...
def ppi():
	...
	colnames = ['PPI Database', 'Jupyter Notebook', 'Total Proteins', 'Interactions', 
	'Hub Proteins (GMT)', 'Average Interactions per Protein', 'Processed Files',
...
def gene_symbol():
	...
	# Subset interactions where selected gene is a source
	source_df = gene_interactions.query('submission_type_fk != 2 and source_gene_fk == @g.index[0]')
	source_df.drop('source_gene_fk', axis =1, inplace=True)
	source_df.drop_duplicates(inplace = True)

	# Subset interactions where selected gene is a target
	target_df = gene_interactions.query("target_gene_fk==@g.index[0] and submission_type_fk != 2")
	target_df.drop('target_gene_fk', axis =1, inplace=True)
	...
	return render_template('gene_symbol.html', gene_s=gene_s, genes = genes, g=g, gene_info=gene_info, source_df=source_df, target_df = target_df, ppi_df=ppi_df)
...
def get_file():
	...
	if labeled_interaction_dataframe.type_name[0] == 'GMT':
		gmt_string = "<br>".join(["\t".join([source_gene_symbol, 'None']+labeled_interaction_dataframe.loc[source_gene_symbol,"target_gene_symbol"].tolist()) for source_gene_symbol in labeled_interaction_dataframe.index.unique()])
	elif labeled_interaction_dataframe.type_name[0] == 'SIG':
